...
const capNhatSoluong = (key, tagSoluong) => {
  ...
  document.getElementById(
    `tt${key}`
  ).innerHTML = `${thanhTien.toLocaleString()}<sup>đ</sup>`;
  tongThanhtien();
};
    ...
    html += `
        <tr class="text-nowrap">
              <td scope="row" class="text-center" style="width:10%">
                <img class="img-fluid" src="${urlImages}/${
      item.maso
    }.png" alt="">
              </td>
              <td>${item.ten}</td>
              <td class="text-right">
                <input type="number" min="1" max="10" value="${
                  item.soluong
                }" onchange="capNhatSoluong('${item.maso}',this)">
              </td>
              <td class="text-right">
              ...
              <td class="text-center">
                <button class="btn btn-danger btn-sm" onclick="xoaSanpham('${
                  item.maso
                }')">
